...
#ASSUNTOS QUE NAO DEVEM TER O DISCLAIMER INSERIDO
#A PRINCIPIO NAO INCLUIR EM REPOSTAS
ASSUNTOS=/etc/postfix/assuntos
...
# FAZ A PESQUISA DO ASSUNTO
assunto_msg=`grep -i -m 1 "Subject:" in.$$ | cut -d " " -f 2 | cut -d " " -f 1`


### FAZ O TRATAMENTO PARA NAO INCLUIR DISCLAIMER NOS ASSUNTOS DESEJADOS

if [ `grep -wi ^${assunto_msg}$ ${ASSUNTOS}` ]; then

$SENDMAIL -oi "$@" <in.$$
exit $?

#TRATA AS EXCECOES PRIMEIRO
elif [ `grep -wi ^${from_address}$ ${EXCECOES_REMETENTES}` ]; then
